...
class Settings(BaseSettings):
    ...
    database_url: str = Field(
        default="postgresql://user:password@localhost/mcp_memory",
        env="DATABASE_URL"
    )
    redis_url: str = Field(
    ...
    memory_relevancy_threshold: float = Field(
        default=0.7,
        env="MEMORY_RELEVANCY_THRESHOLD"
    )
    max_memory_chunk_size: int = Field(
        default=1000,
        env="MAX_MEMORY_CHUNK_SIZE"
    )
    auto_summarize_threshold: int = Field(
        default=50,
        env="AUTO_SUMMARIZE_THRESHOLD"
    )
    max_short_term_memory: int = Field(
        default=100,
        env="MAX_SHORT_TERM_MEMORY"
    )
