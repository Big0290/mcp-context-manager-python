...
from mcp_memory_server.models.memory import (
    Memory, MemoryCreate, MemoryUpdate, MemorySearch, MemoryResponse, MemoryType
)
...
class MemoryEngine:
    ...
    async def get_memory(self, db: Session, memory_id: UUID) -> Optional[MemoryResponse]:
    ...
    async def search_memories(
        ...
        query = db.query(Memory).filter(
            and_(
                Memory.agent_id == search_params.agent_id,
                Memory.is_deleted == False
            )
        )
        ...
        if search_params.query:
            ...
            memories = query.order_by(desc(Memory.created_at)).limit(
                search_params.limit
            ).all()
