...
def test_mcp_server_communication():
    ...
    try:
        ...
        print(
            f"✅ Init response: {result.get('result', {}).get('serverInfo', {}).get('name', 'Unknown')}"
        )
...
def check_cursor_configuration():
    ...
    if config_file.exists():
        ...
        if "mcp-memory-server" in mcp_servers:
            ...
            print(
                f"   Auto injection: {server_config.get('autoContextInjection', False)}"
            )
            print(
                f"   Context project: {server_config.get('contextProjectId', 'Unknown')}"
            )
        else:
