...
async def test_automatic_recording():
    ...
    try:
        ...
        init_response = send_mcp_message(process, {
            "jsonrpc": "2.0",
            "id": 1,
            "method": "initialize",
            "params": {}
        ...
        tools_response = send_mcp_message(process, {
            "jsonrpc": "2.0",
            "id": 2,
            "method": "tools/list",
            "params": {}
        ...
        start_recording_response = send_mcp_message(process, {
            "jsonrpc": "2.0",
            "id": 3,
            "method": "tools/call",
            "params": {
        ...
        if not result.get('isError', True):
            print(f"✅ {result.get('content', [{}])[0].get('text', 'Recording started')}")
        else:
        ...
        context_response = send_mcp_message(process, {
            "jsonrpc": "2.0",
            "id": 4,
            "method": "tools/call",
            "params": {
        ...
        memory_response = send_mcp_message(process, {
            "jsonrpc": "2.0",
            "id": 5,
            "method": "tools/call",
            "params": {
        ...
        stats_response = send_mcp_message(process, {
            "jsonrpc": "2.0",
            "id": 6,
            "method": "tools/call",
            "params": {
        ...
        fetch_response = send_mcp_message(process, {
            "jsonrpc": "2.0",
            "id": 7,
            "method": "tools/call",
            "params": {
        ...
        memories = fetch_response.get('result', {}).get('content', [{}])[0].get('text', '')
        print(f"📝 Retrieved memories: {memories}")
        
        # Stop conversation recording
        print("\n6. Stopping conversation recording...")
        stop_response = send_mcp_message(process, {
            "jsonrpc": "2.0",
            "id": 8,
            "method": "tools/call",
            "params": {
        ...
        if not result.get('isError', True):
            print(f"✅ {result.get('content', [{}])[0].get('text', 'Recording stopped')}")
        else:
