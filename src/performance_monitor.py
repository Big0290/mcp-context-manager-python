...
class PerformanceMonitor:
    ...
    def track_event(self, event_type: str, project_id: str = None, 
                   duration_ms: float = None, success: bool = True,
                   error_message: str = None, context_length: int = None,
                   memory_count: int = None, metadata: Dict = None):
    ...
    def _store_metric(self, metric: Dict[str, Any]):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            metric["timestamp"], metric["event_type"], metric["project_id"],
            metric["duration_ms"], metric["success"], metric["error_message"],
            metric["context_length"], metric["memory_count"], metric["metadata"]
        ))
        
        conn.commit()
        conn.close()
    
    def record_user_feedback(self, feedback_type: str, rating: int, 
                           comment: str = None, project_id: str = None,
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (
            datetime.now().isoformat(), feedback_type, rating, comment,
            project_id, context_summary
        ))
        
        conn.commit()
        conn.close()
    
    def record_usage_pattern(self, action_type: str, project_id: str = None,
                           context_used: bool = False, manual_trigger: bool = False,
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (
            datetime.now().isoformat(), user_id, session_id, action_type,
            project_id, context_used, manual_trigger
        ))
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if success_rate < 90:
            recommendations.append("⚠️ Success rate below 90%. Consider improving error handling.")
        
        # Analyze average duration
        avg_duration = report["overall_stats"]["avg_duration_ms"]
        if avg_duration and avg_duration > 1000:
            recommendations.append("🐌 Context injection taking >1s. Consider optimization.")
        
        # Analyze feedback
        feedback_ratings = [f["avg_rating"] for f in report["feedback_summary"] if f["avg_rating"]]
        if feedback_ratings and sum(feedback_ratings) / len(feedback_ratings) < 4.0:
            recommendations.append("⭐ User feedback below 4.0. Consider improving context quality.")
        
        # Analyze usage patterns
        for pattern in report["usage_patterns"]:
            if pattern["manual_trigger_rate"] > 50:
                recommendations.append("🎮 High manual trigger usage. Consider improving automatic injection.")
...
class ContextInjectionMonitor:
    ...
    def track_context_injection(self, project_id: str, context_length: int, 
    ...
    def track_memory_operation(self, operation_type: str, project_id: str,
                             duration_ms: float, success: bool, memory_count: int = None):
    ...
    def record_context_feedback(self, rating: int, comment: str = None, 
                              project_id: str = None, context_summary: str = None):
...
def main():
    ...
    try:
        ...
        for event in report['event_breakdown']:
            print(f"• {event['event_type']}: {event['count']} events, {event['avg_duration_ms']:.1f}ms avg")
