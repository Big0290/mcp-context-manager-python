...
class BrainEnhancedMCPServer:
    ...
    def __init__(self, enable_brain_features: bool = True):
        ...
        self.brain_integration = BrainIntegration(
            ...
            enable_brain_features=enable_brain_features
        )
    ...
    async def get_brain_status(self):
        ...
        if not self.enable_brain_features:
            return {
                "brain_enabled": False,
                "message": "Brain features are disabled"
            }
        ...
        try:
            ...
            status = {
                ...
                "connections_count": sum(len(conns) for conns in brain_system.connections.values()),
            ...
            if method and method != "initialize":
                ...
                if method == "tools/call":
                    ...
                    if tool_name == "push_memory":
                        ...
                        server.conversation_recorder.record_user_message(f"User searched similar experiences: {query}")
                    elif tool_name == "get_knowledge_graph":
                        topic = arguments.get("center_topic", "")
                        server.conversation_recorder.record_user_message(f"User requested knowledge graph for: {topic}")
                    else:
