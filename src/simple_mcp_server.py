...
from src.performance_monitor import ContextInjectionMonitor, PerformanceMonitor
...
class ConversationRecorder:
    ...
    def end_conversation(self):
        ...
        if self.conversation_buffer:
            ...
            asyncio.create_task(
                self._auto_record_memory("conversation_summary", summary)
            )
    ...
    def _create_conversation_summary(self) -> str:
        ...
        user_messages = [
            msg["content"] for msg in self.conversation_buffer if msg["type"] == "user"
        ]
        ai_responses = [
            msg["content"] for msg in self.conversation_buffer if msg["type"] == "ai"
        ]
...
class SimpleMCPServer:
    def __init__(self):
        ...
        self.logger = logging.getLogger(__name__)
        self.logger.info(
            f"Simple MCP Server initialized for project: {self.project_id}"
        )
    ...
    async def _fetch_memory(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if tags:
                ...
                params.append(
                    f"%{json.dumps(tags)[1:-1]}%"
                )  # Escape JSON tags for LIKE
        ...
        try:
            ...
            if focus_areas:
                ...
                for memory in project_memories:
                    ...
                    if any(
                        area.lower() in [tag.lower() for tag in memory_tags]
            ...
            if project_memories:
                high_priority = [
                    m for m in project_memories if m[3] == "high"
                ]  # priority
                if high_priority:
        ...
        try:
            ...
            if report["usage_patterns"]:
                ...
                for pattern in report["usage_patterns"]:
                    report_text += (
                        f"• {pattern['action_type']}: {pattern['count']} uses\n"
                    )
                    report_text += (
                        f"  - Context used: {pattern['context_used_rate']:.1f}%\n"
                    )
                    report_text += (
                        f"  - Manual triggers: {pattern['manual_trigger_rate']:.1f}%\n"
                    )
                report_text += "\n"
    ...
    async def _record_feedback(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "content": [
                    {"type": "text", "text": f"Error recording feedback: {str(e)}"}
                ],
                "isError": True,
            }
    ...
    async def _stop_conversation_recording(
        ...
        try:
            ...
            user_messages = len(
                [
                    msg
                    for msg in self.conversation_recorder.conversation_buffer
                    if msg["type"] == "user"
                ]
            )
            ai_responses = len(
    ...
    async def _craft_ai_prompt(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            context = PromptContext(
                project_id=project_id, focus_areas=focus_areas, prompt_type=prompt_type
            )
            ...
            return {
                "content": [
                    {"type": "text", "text": f"Error crafting AI prompt: {str(e)}"}
                ],
                "isError": True,
            }
    ...
    async def _auto_inject_context(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if show_notification:
                notification = (
                    f"📋 **Automatic Context Injection**\n\nProject: {project_id}\n\n"
                )
            else:
                notification = ""

            return {
                "content": [{"type": "text", "text": f"{notification}{context_text}"}],
                "isError": False,
            }
