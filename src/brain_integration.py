...
from .brain_memory_system import (
    BrainMemorySystem, MemoryNode, MemoryLayer, MemoryState, 
    ConnectionType, MemoryMetadata
)
...
class BrainIntegration:
    ...
    def __init__(self, original_mcp_server, enable_brain_features: bool = True):
        ...
        if self.enable_brain_features:
            self.brain_system = BrainMemorySystem(
                db_path=str(Path(original_mcp_server.db_path).parent / "brain_memory.db"),
                embedding_service=getattr(original_mcp_server, 'embedding_service', None)
            )
        else:
    ...
    async def execute_enhanced_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if self.enable_brain_features and tool_name in [
            "search_similar_experiences", "get_knowledge_graph", "get_memory_insights",
            "promote_memory_knowledge", "trace_knowledge_path"
        ]:
    ...
    async def _execute_brain_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _enhance_pushed_memory(self, arguments: Dict[str, Any], result: Dict[str, Any]):
    ...
    async def _get_context_summary_enhanced(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Add brain insights
            brain_insights_text = "\n\n🧠 **Memory Insights:**\n"
            brain_insights_text += f"• Memory Layers: {dict(insights['layer_distribution'])}\n"
            brain_insights_text += f"• Memory States: {dict(insights['state_distribution'])}\n"
            brain_insights_text += f"• Top Topics: {dict(list(insights['top_topics'].items())[:3])}\n"
            brain_insights_text += f"• Connection Types: {dict(insights['connection_patterns'])}\n"
    ...
    async def _search_similar_experiences(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Format response
            response_parts = []
            response_parts.append(f"🔍 **Similar Experience Search Results for:** '{query}'")
            ...
            if direct_matches:
                ...
                for result in direct_matches[:3]:
                    node = result["node"]
                    response_parts.append(f"• [{node.metadata.memory_layer.value.upper()}] {node.content[:100]}...")
                    response_parts.append(f"  Score: {result['score']:.2f} | Layer: {node.metadata.memory_layer.value}")
                response_parts.append("")
            ...
            if similar_experiences:
                ...
                for result in similar_experiences[:3]:
                    node = result["node"]
                    response_parts.append(f"• [{node.metadata.memory_layer.value.upper()}] {node.content[:100]}...")
                    response_parts.append(f"  Similarity: {result['score']:.2f} | Used {node.metadata.access_count}x")
                response_parts.append("")
            ...
            if connected_knowledge:
                ...
                for result in connected_knowledge[:3]:
                    node = result["node"]
                    response_parts.append(f"• [{result['match_type'].upper()}] {node.content[:100]}...")
    ...
    async def _find_analogical_patterns(self, results: List[Dict[str, Any]]) -> List[str]:
        ...
        try:
            ...
            for memory_id in [r["memory_id"] for r in related_memories[:10]]:
                if memory_id in self.brain_system.connections:
                    for target_id, connection in self.brain_system.connections[memory_id].items():
            ...
            for layer, node_ids in graph["layers"].items():
                response_text += f"**{layer.title()} Layer:** {len(node_ids)} memories\n"
    ...
    async def _promote_memory_knowledge(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _trace_knowledge_path(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response_parts = []
            response_parts.append(f"🛤️ **Knowledge Path: {from_concept} → {to_concept}**")
    ...
    async def _find_shortest_path(self, start_id: str, end_id: str, max_hops: int) -> Optional[List[str]]:
        ...
        while queue:
            ...
            if current_id in self.brain_system.connections:
                for target_id, connection in self.brain_system.connections[current_id].items():
                    if target_id == end_id:
