...
def update_schema():
    ...
    try:
        ...
        if "agent_id" not in columns:
            ...
            cursor.execute(
                "ALTER TABLE memories ADD COLUMN agent_id TEXT DEFAULT 'cursor-agent'"
            )
        ...
        if "custom_metadata" not in columns:
            ...
            cursor.execute(
                "ALTER TABLE memories ADD COLUMN custom_metadata TEXT DEFAULT '{}'"
            )
        ...
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='brain_synapses'"
        )
        if not cursor.fetchone():
