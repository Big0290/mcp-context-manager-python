...
class BrainFeatureEnabler:
    ...
    def __init__(self, project_root: Path = None):
        ...
        self.backup_dir = self.project_root / "backup" / f"brain_upgrade_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ...
            try:
                ...
                if "mcpServers" in config:
                    for server_name, server_config in config["mcpServers"].items():
                        if "mcp-context-manager" in server_name or "simple_mcp" in server_name:
