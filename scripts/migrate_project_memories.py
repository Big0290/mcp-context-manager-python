...
class MemoryMigrator:
            ...
            try:
                with sqlite3.connect(simple_db_path) as conn:
                    ...
                    if any('memories' in table[0].lower() for table in tables):
                        cursor.execute("SELECT DISTINCT project_id FROM memories WHERE project_id != ?", 
                                     (self.new_project_id,))
    ...
    def backup_databases(self) -> bool:
        ...
        try:
            ...
            simple_db_path = Config.SIMPLE_DB_PATH
            if simple_db_path.exists():
                backup_file = backup_path / f"simple_mcp_memory_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
                shutil.copy2(simple_db_path, backup_file)
                self.migration_log.append(f"Backed up simple database to: {backup_file}")
            
            # Backup full database
            full_db_path = Config.FULL_DB_PATH
            if full_db_path.exists():
                backup_file = backup_path / f"mcp_memory_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"
        ...
        try:
            with sqlite3.connect(simple_db_path) as conn:
                ...
                for old_project_id in old_project_ids:
                    ...
                    self.migration_log.append(f"Migrated {migrated_count} memories from '{old_project_id}' to '{self.new_project_id}'")
                ...
                for old_project_id in old_project_ids:
                    ...
                    for memory in memories:
                        try:
                            ...
                            """, (
                                memory[0], memory[1], self.new_project_id, memory[2],
                                memory[3], memory[4], memory[5], memory[6], memory[7],
                                memory[8], memory[9], memory[10], memory[11], memory[12]
                            ))
                            migrated_count += 1
    ...
    def validate_migration(self) -> bool:
        ...
        try:
            ...
            if simple_db_path.exists():
                with sqlite3.connect(simple_db_path) as conn:
                    cursor = conn.cursor()
                    cursor.execute("SELECT COUNT(*) FROM memories WHERE project_id = ?", 
                                 (self.new_project_id,))
                    simple_count = cursor.fetchone()[0]
                    self.migration_log.append(f"Simple database: {simple_count} memories for new project ID")
            ...
            if full_db_path.exists():
                with sqlite3.connect(full_db_path) as conn:
                    ...
                    self.migration_log.append(f"Full database: {full_count} memories for new project ID")
    ...
    def run_migration(self, dry_run: bool = False) -> bool:
        ...
        print(f"📋 Found {len(old_project_ids)} old project IDs: {', '.join(old_project_ids)}")
...
def main():
    ...
    parser.add_argument("--new-project-id", default="mcp-context-manager-python",
    ...
    if success:
        ...
        if not args.dry_run:
            print("🔄 You may need to restart your MCP server for changes to take effect")
    else:
        print("\n❌ Migration failed!")
        sys.exit(1)
