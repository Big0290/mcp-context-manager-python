...
class CommunityManager:
    ...
    def get_contributors(self) -> List[Dict]:
        ...
        if response.status_code == 200:
            ...
            console.print(f"[red]Error fetching contributors: {response.status_code}[/red]")
            return []
    ...
    def generate_community_report(self) -> None:
        ...
        console.print(Panel.fit("🧠 MCP Context Manager - Community Report", style="bold blue"))
    ...
    def _generate_insights(self, contributors: List[Dict], issues: List[Dict], prs: List[Dict]) -> None:
        ...
        if top_contributor:
            console.print(f"🏆 Top Contributor: {top_contributor['login']} ({top_contributor['contributions']} contributions)")
        
        # Issue insights
        good_first_issues = [i for i in issues if any(label["name"] == "good first issue" for label in i.get("labels", []))]
        help_wanted_issues = [i for i in issues if any(label["name"] == "help wanted" for label in i.get("labels", []))]
    ...
    def _generate_recommendations(self, contributors: List[Dict], issues: List[Dict], prs: List[Dict]) -> None:
