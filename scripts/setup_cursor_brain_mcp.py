...
class CursorBrainMCPSetup:
    ...
    def _get_cursor_config_paths(self):
        ...
        if self.system == "darwin":  # macOS
            return [
                Path.home() / "Library" / "Application Support" / "Cursor" / "User" / "globalStorage" / "rooveterinaryinc.cursor-mcp" / "settings.json",
                Path.home() / ".cursor" / "mcp_settings.json",
                Path.home() / ".config" / "cursor" / "mcp_settings.json"
            ]
        elif self.system == "windows":
            return [
                Path.home() / "AppData" / "Roaming" / "Cursor" / "User" / "globalStorage" / "rooveterinaryinc.cursor-mcp" / "settings.json",
                Path.home() / ".cursor" / "mcp_settings.json"
            ]
        else:  # Linux
            return [
                Path.home() / ".config" / "Cursor" / "User" / "globalStorage" / "rooveterinaryinc.cursor-mcp" / "settings.json",
                Path.home() / ".cursor" / "mcp_settings.json",
                Path.home() / ".config" / "cursor" / "mcp_settings.json"
            ]
    ...
    def test_brain_server_imports(self):
        ...
        try:
            ...
            from src.brain_memory_system import BrainMemorySystem, MemoryLayer, MemoryState
            ...
            try:
                ...
                if config_path.exists():
                    try:
                        ...
                        print(f"   ⚠️  Invalid JSON in existing config, creating new one")
                        existing_config = {}
...
'''
    ...
    def test_server_startup(self):
        ...
        try:
            ...
            result = subprocess.run([
                sys.executable, 
                str(self.project_root / "test_brain_mcp.py")
            ], 
            capture_output=True, 
            text=True, 
            timeout=30,
    ...
    def print_instructions(self, config_installed, successful_paths):
        ...
        print(f"• Startup script: {self.project_root}/start_brain_mcp.{'sh' if self.system != 'windows' else 'bat'}")
